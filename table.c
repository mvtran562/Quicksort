...
// Partition Function
// This function will find where the pivot point will be
// It will loop through the passed in array
// It will compare the current iteration to the last item
// if it is less than the last, they will swap
// after the loop it will swap the last item with wherever the sep variable lands at
// O(n)
static int partition(void **a,int(*compare)(),int lo, int hi)
{
    int i;
    int sep;
    void *temp;
...
// The function wll recurssively sort the left and right of the partition
// It will go until the lo is equal to or greater than hi
// avg O(nlogn) worst O(n^2)
static void quicksort(void **elts, int(*compare)(), int lo, int hi)
{
    if(lo < hi)
    {
        int ploc = partition(elts,compare,lo, hi);
        quicksort(elts,compare,lo,ploc-1);
        quicksort(elts,compare,ploc+1,hi);
    }
}
...
void *getElements(SET *sp)
{
    ...
    quicksort(elts,sp->compare,0,sp->count-1);
    return elts;
}